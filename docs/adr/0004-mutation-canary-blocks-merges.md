...
runner refuses unknown arguments rather than growing a filter flag. Cost is controlled only
by the job-level path gate in `.gitlab-ci.yml`: MRs touching none of `backend/`, `shared/`,
`scripts/`, the root lockfile or the CI file skip the whole job, all-or-nothing.
