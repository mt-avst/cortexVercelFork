...
- [ ] **Native poll and survey deferrals (recorded 2026-08-17, phases 4a-4d security reviews)** – Found by the gates, judged not to block the phase they were found in:
  - **No rate limit on either participant session-mint route** (`/:id/recorded-study-session`, `/:id/survey-session`). 60 sessions minted in under a second from one cookie, measured. Survey minting is idempotent per participant per opportunity as of 4d, which removes most of the value of abusing it, but the ceiling is still absent and the limiter pattern already exists in the same file (`recordedStudyBriefLimiter`). Opportunity WRITE routes are equally unlimited, and each `inline_survey` write inserts a study plus up to 51 step rows on the 5-connection FirstHand runtime pool shared with live participant sessions.
  ...
  - **`SurveyRunner` sends no `link_opened` or `session_started`**, so a native survey's funnel shows completions with no starts.
  ...
  - **`/dev/survey-preview` and its `VITE_SURVEY_PREVIEW` flag are dead**, and as of 4e completely so: the harness exists to preview the runner and the results view before either had a route, and both now have one (`/survey/:token` and the Responses tab). Its own docblock says to delete it once that is true. Left in place only because it was explicitly out of 4e's scope.
...
- [ ] **Phase 4e gate deferrals (recorded 2026-08-17, code review + security review, both at Opus)** – Raised against the per-opportunity results routes, judged not to block them. Neither gate found a CRITICAL, and the security gate could not break the authorisation boundary: it proved all four design constraints by mutation and by live queries against the database.
  - **No rate limit on either results route, and no `LIMIT` on the projection.** `listResponsesForOpportunity` returns every answer for the opportunity, on the FirstHand runtime pool that is capped at 5 connections and shared with live participant sessions, so an admin looping their own export contends for them. Same class as the mint-route deferral above, and the limiter pattern already exists in the same file.
  - **Answers are re-identifiable by the same principal, and one piece of copy implies otherwise.** The results projection carries `session_id` (it is also the CSV's Participant column), and `GET /api/opportunities/:id/session-events` — same owner gate — carries `firsthand_session_id` beside `participant_name` and `participant_email`, so the join from an answer to a named employee is exact. The owner is entitled to both sets and the product makes no anonymity claim, but `backend/src/db/reset-demo-data.ts` ships participant-facing copy reading "Your anonymous responses". That wants a **product decision**, not a code change: either drop the word, or emit a per-opportunity salted digest of the session id instead of the raw one.
