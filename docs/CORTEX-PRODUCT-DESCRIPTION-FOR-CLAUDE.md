...
- ⚠️ **Any new file under `shared/firsthand/` must be added to `frontend/src/shared/__tests__/shared-copies-are-current.test.ts`'s `copies` list.** `copy-shared-types.js` copies that directory wholesale while the test enumerates it by hand, so a new file is copied and never verified — `consent-templates.ts` shipped that way, and deleting its cross-kind guard from the frontend copy alone passed the whole frontend suite. C1 added a directory-comparison guard, but the list is still hand-written.
...
**The two backward concepts are named apart.** The control that leaves the form and the control that goes back one step were two `btn-outline-secondary` buttons carrying the same `ArrowLeft` and the same word, told apart by position alone. The top one is now **"Exit to dashboard"** (or "Exit to home" when `allowUserSubmission`), with a different icon and an unsaved-changes confirmation; the bottom one is **"Previous: {step name}"**, derived from the step list rather than a number hard-coded at each call site, across **all six** of them — `AdminSessionManager` renders the sixth and had been missed.
