...
- **Type:** `test`, `poll`, `survey`, `interview`, `question`, `unmoderated`. `question` points at an external tool. `poll` and `survey` do either, per `delivery_mode` (see *Native polls and surveys* below): `external` hands off to SurveyMonkey and friends, `native` runs the questions inside Cortex. `unmoderated` is a self-guided study that Cortex records in the browser (see *Unmoderated studies* below). `test` and `interview` are bookable, and since #79 (7.65.0) they carry opportunity-anchored consent and hold post-session artefacts — the researcher ingests the call platform's own recording/transcript export against the booking after the call.
- **Content:** title, purpose one-liner, optional description, optional product, default duration.
  ...
  A recorded session carries its own **optional** duration on the Task List instead (`firsthand.studies.estimated_duration_minutes`, nullable, no default).
...
### Unmoderated studies (participant-facing name: **Recorded session**)
...
- **Vocabulary** — admin-side, the script object is a **Task List** and the word *study* means the research project (the opportunity). Participant-facing copy says **"Recorded session"**, deliberately: a participant only ever meets one object, so the distinction would be noise to them. It is named as a pair with `test` (**Live session**) because the two are one research method differing only in whether a researcher is present — the earlier "Usability test"/"Recorded study" pair implied the recorded one was not a usability test. The word *Recorded* is load-bearing rather than decorative: it is the first and most-repeated part of the recording disclosure `RecordedStudyExpectations` completes on the detail page, and it reaches a participant on the browse row and the booking card long before that page does.
- **Participation** — the participant opens the published opportunity, clicks *Start recorded session*, consents, then opens the floating task panel and drives the rest from there: the task page opens in its own window (so it appears in the browser's share picker), then recording starts. A session is minted server-side and its steps are **snapshotted at launch**, so editing a Task List never changes a session already in flight.
...
- **Who can take part** — logged-in Cortex users only. There is no anonymous or emailed-token route, so a recorded session cannot be sent to anyone outside the organisation.
...
- **The step set changed.** The two authoring paths gained a **Consent** step — Basic information → Content → Questions/Task List → Consent — and, since a **Review** step was later added as the final step, the create/update control lives on **Review** (the only step that commits), not on Consent. The **Status (draft/published) control moved from Basic Information onto that Review step** with #111 (2026-09-07): publishing is now the last decision after configuration, and the participant share link surfaces on Review once the study is published (#108). The two MODERATED paths (test/interview) gained a Consent step of their own with #79 (7.65.0): Cortex now stores a moderated session's recording and transcript against the booking, so what the participant agrees to at booking time is what Cortex may hold afterwards — the consent is anchored to the opportunity (`consent_*` columns, refused on every other type), acceptance is recorded on the booking with the wording snapshotted and artefact ingest refuses without acceptance or a typed attestation. Only the pure hand-offs (external poll/survey/question) remain at three steps with no Consent step: there is no consent for Cortex to govern when the participant answers in somebody else's tool and Cortex keeps no artefacts. `getTabsForType` derives the Consent step from the presence of a `questions`/`taskList` step (so a future authoring type gets one for free) and adds it by type for the two moderated shapes.
- **Three templates, versioned.** `shared/firsthand/consent-templates.ts` holds `recorded-default` v1, `survey-default` v1 (the two strings that were already the hardcoded defaults) and, since #79, `moderated-default` v1 (**Standard live-session consent**) — each with an id, a version, a name and a summary. A study stores `firsthand.studies.consent_template_id` and `consent_template_version`, or `custom` with a null version; a moderated opportunity has no study row, so its pair lives on `opportunities.consent_template_id`/`consent_template_version` beside `consent_text`.
- 🔑 **The classification is VERIFIED, never trusted.** `resolveConsentTemplate` takes a client's *claim* about which template its wording is and returns it **only when the wording actually is that template version's wording**; otherwise `custom`. So a caller can always understate its approval and can never overstate it. A claim naming the other kind's template is refused before it is even compared — the survey wording's central sentence is "Nothing is recorded", which would be false of a recorded session.
...
- **Browse:** A single **index of rows** (not a card grid) of published opportunities, filtered by type. Sorted **closing soonest first**, with unknown deadlines after those and ended studies last. Each row carries a verb for what taking part involves (*Book a time*, *Open poll*, *Start recorded session*), and participant-facing type names — **Recorded session**, not "unmoderated".
  ...
  `test` and `unmoderated` are named as a **pair** — **Live session** and **Recorded session** — because they are one research method differing only in whether a researcher is present. "Usability test" and "Recorded study" implied they were two different kinds of study.
- **Opportunity detail:** Full description, sessions with remaining slots, Book button (bookable types) or Open Poll/Survey (external link types).
  Eligibility is stated **only when it narrows** who can take part (`external`, or `specific` with the researcher's criteria) — `getEligibilityNote`, shared with the browse row. "Any" is never shown: every route to taking part needs a signed-in Cortex account, and a recorded session refuses external participants outright.
...
- **Create & Manage** (`/admin`, the researcher workspace, called "Admin" until #169): Counts (opportunities, bookings, participants, available slots); table of opportunities with stats (sessions, bookings, clicks for poll/survey).
...
- **Analytics (per opportunity):** For poll/survey/unmoderated (and relevant types): views/actions, time-series, conversion; period 7/14/30 days. Admin/owner only. Unmoderated additionally has per-session review — recording playback and transcript (answers are spoken, so no typed responses are stored). Test/interview have a **Participants tab** instead (#79): the booking roster, a private researcher note per booking and the artefact section — upload of the call platform's recording/transcript export, gated `<video>` playback, inline transcript rendering and delete-with-confirm. Consent surfaces inside the expanded artefact panel: a booking with no recorded acceptance shows a banner and upload asks for a typed attestation that consent was obtained outside Cortex.
