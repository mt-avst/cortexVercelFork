...
//   components/admin/ already holds five extractions in the same spirit
//   (PhoneStudyFilters, kebabOrder, AdminSnapshotStrip, buildTableRows,
//   StudyRow - cto/AdaptaLabs#163 pulled out the last two). What is left is
//   the page's own state, data loading, filters and the table shell itself
//   - no single further cut is as self-contained as those five were.
...
import { StudyRow } from '../components/admin/StudyRow';
import { buildTableRows } from '../components/admin/buildTableRows';
import { logger } from '../utils/logger';
import PendingApprovals from '../components/PendingApprovals';
import AdminFeedback from '../components/AdminFeedback';
import ErrorState from '../components/ErrorState';
import ConfirmationModal from '../components/ConfirmationModal';
import { Icon, SortCaret } from '../components/ui';
import { Settings, Clock, List, History, MessageSquare, Calendar, Download, Clapperboard, Flag, ArrowRight, CheckCircle, AlertTriangle } from 'lucide-react';
...
const Admin: React.FC = () => {
  ...
  // What the table draws - buildTableRows.ts (cto/AdaptaLabs#163) for the
  // three in-place exceptions this holds while a Close/Undo/Reopen notice is
  // up. Still a `useMemo` here, with the same dependency list, over a pure
  // function there.
  const tableRows = useMemo(
    () =>
      buildTableRows({
        frozen: closeUndo.frozenSnapshot,
        errorAnchor: closeUndo.actionError?.anchor ?? null,
        reopenSnapshot: closeUndo.reopenNotice?.snapshot ?? null,
        sortedOpportunities,
                  ...
                  {!loadingOpportunities && !error && tableRows.length > 0 && (
                    <div className="table-responsive" style={{
                      ...
                      >
                        ...
                        <tbody>
                          {tableRows.map(({ study, kind }) => (
                            <StudyRow
                              key={
                                kind === 'errorSlot'
                                  ? `${study.id}-error-slot`
                                  : kind === 'reopenNoticeOnly'
                                  ? `${study.id}-reopen-notice-only`
                                  : study.id
                              }
                              study={study}
                              kind={kind}
                              ...
                              refreshAfterCopy={refreshAfterCopy}
                              copyRetryRef={copyRetryRef}
                              dismissCopyNotice={dismissCopyNotice}
