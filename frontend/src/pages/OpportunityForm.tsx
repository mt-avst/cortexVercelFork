...
const OpportunityForm: React.FC<{ allowUserSubmission?: boolean }> = ({ allowUserSubmission = false }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent, skipNavigation = false): Promise<string | undefined> => {
    ...
    try {
      ...
      if (['poll', 'survey', 'question', 'unmoderated'].includes(formData.type)) {
        console.log('📅 Date values from formData - start_date:', formData.start_date, '| end_date:', formData.end_date);
        console.log('📅 start_date type:', typeof formData.start_date, '| end_date type:', typeof formData.end_date);
        console.log('📅 start_date truthy:', !!formData.start_date, '| end_date truthy:', !!formData.end_date);
        data.start_date = formData.start_date || undefined;
        data.end_date = formData.end_date || undefined;
        console.log('📅 Date values being sent - start_date:', data.start_date, '| end_date:', data.end_date);
      }
