...
// Glowing embers aesthetic, max lightness 0.6
const getNodeColorHSL = (): THREE.Color => {
  ...
  if (rand < 0.55) {
    // 55% dim embers - deep background
    lightness = 0.1 + Math.random() * 0.1;
  } else if (rand < 0.85) {
    // 30% warm glow
    lightness = 0.25 + Math.random() * 0.15;
  } else {
    // 15% bright embers (but NOT white)
    lightness = 0.45 + Math.random() * 0.15; // Max 0.6
  }
...
// ============================================================================
// Simplex-like 3D Noise (simplified)
// ============================================================================
...
interface NodeData {
  ...
  basePositions: Float32Array; // For breathing animation
  colors: Float32Array;
  scales: Float32Array;
  isHotspot: boolean[];
  noiseSeeds: Float32Array; // Unique seed per node
}
...
const generateVolumetricNodes = (count: number): NodeData => {
  ...
  for (let i = 0; i < count; i++) {
    // Random point in thick shell volume (not surface)
    const theta = Math.random() * Math.PI * 2;
    const phi = Math.acos(2 * Math.random() - 1);
    
    // Radius between inner and outer (volumetric, not surface)
    const t = Math.random();
    const radius = CONFIG.innerRadius + t * (CONFIG.outerRadius - CONFIG.innerRadius);
    
    // Apply noise displacement for organic clustering
    ...
    // Hotspots are bright core nodes
    ...
    // Color assignment - amber embers, no white
    let color: THREE.Color;
    if (hotspot) {
      // Hotspots are brighter amber (but not white)
      color = new THREE.Color();
      color.setHSL(0.08, 1.0, 0.5 + Math.random() * 0.1); // Max 0.6
    } else {
...
interface ConnectionData {
  positions: Float32Array;
  colors: Float32Array;
  indices: Uint32Array; // For signal routing
}
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    for (let j = i + 1; j < nodeCount; j++) {
      ...
      if (dist < threshold) {
        ...
        // Store indices for signal routing
        connectionIndices.push(i, j);

        // Color with distance-based fade (darker at edges)
...
const Nodes: React.FC<NodesProps> = ({ nodeData }) => {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Direction from center for breathing
...
const SignalPackets: React.FC<SignalPacketsProps> = ({ connectionData, nodeData }) => {
  ...
  // Signal state: which connection, progress along it
  ...
  useFrame(() => {
    ...
    signalState.current.forEach((signal, i) => {
      ...
      // Reset to new random connection when done
      ...
      // Scale: larger in middle of journey - bright sparks
      const pulseScale = 0.22 + Math.sin(t * Math.PI) * 0.12;
...
const OrganicNeuralBackground: React.FC = () => {
  return (
    <div 
