...
import { getLeaderboard } from '../../shared/services/gamification';
 ...
 * Get the global leaderboard (top users by total AdaptaBits).
 *
 * Query params:
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await getLeaderboard(pool, limit);
    ...
    return res.status(500).json(
      createSafeErrorResponse(error, { userMessage: 'Failed to fetch leaderboard' })
    );
  }
}
