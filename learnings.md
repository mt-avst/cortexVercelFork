...
- **Name**: AdaptaLabs (adaptalabs-root), **version**: 7.3.13
...
- **New studies not visible to users - UX fix (2026-03-19)**: Studies created with status='draft' are not visible to non-admin users, but admins were missing the status dropdown or not understanding its importance. **Fix**: Added prominent draft warnings throughout the form: (1) Status dropdown help text shows orange warning "⚠️ DRAFT - Not visible to users" when draft is selected; (2) Success messages after save show warning banner with AlertTriangle icon when saving as draft: "⚠️ Study created as DRAFT - Not visible to users yet. Change status to Published to make it visible."; (3) Alert styling changes from green (success) to yellow (warning) for draft saves; (4) Longer display time (3s vs 1.5s) for draft warnings so admins notice them. Files modified: `OpportunityForm.tsx` (lines 481-487, 496-502, 509-520, 663-678), `BasicInfoTab.tsx` (lines 109-111).
