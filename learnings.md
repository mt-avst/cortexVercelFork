...
- **Name**: AdaptaLabs (adaptalabs-root), **version**: 7.3.15
...
- **Booking calendar: 30m slots looked as tall as 1h (2026-03-20)**: `CalendarGrid.tsx` set slot `height` to `max(percentage, 40px)`. A 30m slot is ~3.1% of the day column (~28px on a 900px timeline); the **40px floor** dominated, so short slots were inflated to almost the same visual size as 60m (~6.2%, ~56px). **Fix**: use `height: ${roundedHeight}%` only so duration maps to proportional height.
