...
- **New studies not visible to users - UX fix (2026-03-19)**: Studies created with status='draft' are not visible to non-admin users, but admins were missing the status dropdown or not understanding its importance. **Initial fix**: Added prominent draft warnings throughout the form. **Code review fixes**: (1) Added draft warnings for test/interview types via AdminSessionManager - passes `isDraft` prop and navigation state message to Admin.tsx which displays warnings; (2) Made warning messages consistent across all flows; (3) Replaced hardcoded color `#d97706` with Bootstrap `text-warning` class; (4) Made edit mode timeout conditional (3s for draft, 1.5s for published). **Result**: All opportunity types now show clear, consistent draft warnings with proper styling and timing. Files modified: `OpportunityForm.tsx`, `BasicInfoTab.tsx`, `AdminSessionManager.tsx`, `Admin.tsx`.
