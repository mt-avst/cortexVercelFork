...
- **Name**: AdaptaLabs (adaptalabs-root), **version**: 7.3.16
...
- **Booking calendar: 30m slots looked as tall as 1h (2026-03-20)**: (1) `CalendarGrid.tsx` had `max(percentage, 40px)` on slot height — **fixed** by using `%` only. (2) **Root cause that persisted**: `_components.css` applies `.calendar-hud .calendar-slot { min-height: 44px !important; padding: 12px 8px !important; }` (tile/touch targets). `CalendarGrid` uses `calendar-hud`; ~30m slots are ~3% of the column (~28px). **min-height: 44px** overrode that and made short slots nearly as tall as 60m (~6%). **Fix**: `.calendar-hud .calendar-timeline-container .calendar-slot { min-height: 0 !important; padding: 3px 6px !important; }` so duration-proportional height wins.
