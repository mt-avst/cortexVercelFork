...
describe("quoting", () => {
  ...
  it("quotes a prompt containing a comma", () => {
    ...
    expect(rows(csv)[0]).toBe('Session,Participant,Superseded,"Easy, or hard?"');
  });
});
...
describe("formula injection", () => {
  ...
  it("neutralises a formula in a researcher-authored prompt", () => {
    ...
    expect(rows(csv)[0]).not.toMatch(/^Session,Participant,Superseded,"?=/);
...
/**
 * THE EXPORT KEEPS EVERY ANSWER AND FLAGS THE ONES THAT LOST
 * (cto/AdaptaLabs#152).
 *
 * The results page counts one answer per person per question - the latest.
 * The CSV keeps every session's row, names the person beside it and marks a
 * session holding an answer the page replaced, so somebody answering twice is
 * visible rather than silently deduplicated.
 */
describe("one person answering in two sessions", () => {
  const steps = [
    step({ step_id: "q1", type: "single_choice", prompt: "Which?", options: ["Left", "Right"] }),
    step({ step_id: "q2", type: "single_choice", prompt: "And?", options: ["Yes", "No"] })
  ];
  ...
  const answer = (
    ...
    response(stepId, sessionId, { selectedOption: option }, {
  ...
  it("puts one person's sessions next to each other, whoever answered in between", () => {
    ...
    expect(rows(csv).slice(1).map((line) => line.split(",")[0])).toEqual([
      "p-monday",
      "p-tuesday",
      "other"
    ]);
