 ...
 * Run with: DATABASE_URL=<production_url> npx tsx backend/src/db/reset-production-db.ts
 */
async function resetProductionDatabase() {
  ...
  const tls = applyDbTls(cleanUrl, process.env, 'reset-production-db');
  const pool = new Pool({
  ...
  try {
    ...
    // Try to find an admin user by email
    ...
    if (userResult.rows.length > 0) {
      ...
      console.log(`✅ Found admin user: ${adminEmail} (${ownerUserId})`);
    } else {
      ...
      if (adminRoleResult.rows.length > 0) {
        ...
        console.log(`✅ Found admin user: ${adminRoleResult.rows[0].email} (${ownerUserId})`);
      } else {
        ...
        const newUserResult = await client.query(
          `INSERT INTO users (email, name, role) 
           VALUES ($1, $2, $3) 
           RETURNING id`,
          [adminEmail, 'Admin User', 'researcher_admin']
        );
        ownerUserId = newUserResult.rows[0].id;
        console.log(`✅ Created admin user: ${adminEmail} (${ownerUserId})`);
      }
    }
    ...
    console.log(`📊 Created ${opportunities.length} opportunities with ${opportunities.length * 21} total sessions`);
    console.log('📅 All sessions are available for booking starting tomorrow');
