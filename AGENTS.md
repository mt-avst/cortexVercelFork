...
Running it LOCALLY needs a real Postgres. Of the 201 entries, 16 are
database-backed. By FILE PATH they split 187 `backend/`, 12 `shared/`, 2
`frontend/`; by `project` - a different thing, and a real manifest field since
!307 - it is 199 backend to 2 frontend, because the 12 `shared/` entries run
